...
def gen_result(conf_filepath, result_filepath):
    ...
    for dataset in conf.keys():
        for model_name in tqdm(conf[dataset].keys(), desc="model"):
            for bit in conf[dataset][model_name].keys():
                para = conf[dataset][model_name][bit]
                RMSE, MAE, precisions, recalls, FPRs, TPRs, ndcgs, pr_ks, roc_ks, ndcg_ks = gen_result_inner(dataset,
                                                                                                             model_name,
                                                                                                             bit, para)
...
if __name__ == "__main__":
    # gen_result('conf/conf_rmse.yaml', 'gen/result_rmse.json')
    plot('gen/result_rmse.json')
